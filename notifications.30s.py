#!/usr/bin/env PYTHONIOENCODING=utf8 /usr/local/bin/python3
...
enterprise_api_url = os.getenv(
    'GITHUB_ENTERPRISE_API', 'https://github.example.com/api/v3')
...
def print_notifications(notifications, enterprise=False):
    ...
    groups = groupby(
        notifications,
        key=lambda notification: notification['repository']['full_name'])
    for repo, repo_notifications in groups:
        if repo:
            ...
            for notification in repo_notifications:
                ...
                print_bitbar_line(refresh='true', **get_dict_subset(
                    formatted_notification,
                    'title', 'href', 'image', 'templateImage'))
                print_bitbar_line(
                    refresh='true',
                    ...
                    param3='--enterprise' if enterprise else None,
                    **get_dict_subset(formatted_notification,
                                      'image', 'templateImage')
                )
...
def format_notification(notification):
    ...
    latest_comment_url = notification.get(
        'subject', {}).get('latest_comment_url', None)
    typejson = make_github_request(formatted['href'])
    if latest_comment_url:
        formatted['href'] = (
            make_github_request(latest_comment_url) or {}).get(
...
if len(sys.argv) > 1:
    ...
    if command == 'readrepo':
        url = '%s/repos/%s/notifications' % (
            enterprise_api_url if enterprise
            else 'https://api.github.com', args[0])
        print('Marking %s as read' % url)
        make_github_request(url=url, method='PUT',
                            data={}, enterprise=enterprise)
    elif command == 'readthread':
        url = args[0]
        print('Marking %s as read' % url)
        make_github_request(url=url, method='PATCH',
                            data={}, enterprise=enterprise)

else:
    is_github_defined = len(github_api_key) == 40
    is_github_enterprise_defined = len(enterprise_api_key) == 40
    github_notifications = get_notifications(
        enterprise=False) if is_github_defined else []
    enterprise_notifications = get_notifications(
        enterprise=True) if is_github_enterprise_defined else []
    has_notifications = len(github_notifications) + \
        len(enterprise_notifications)
    ...
    if is_github_defined:
        if len(github_notifications):
            print_bitbar_line(
                title=(u'GitHub \u2014 %s' % plural(
                    'notification',
                    len(github_notifications))),
                color=active,
    ...
    if is_github_enterprise_defined:
        if len(enterprise_notifications):
            if is_github_defined:
                print('---')
            print_bitbar_line(
                title=(u'GitHub:Enterprise \u2014 %s' % plural(
                    'notification',
                    len(enterprise_notifications))),
                color=active,
                href='%s/notifications' % re.sub('/api/v3',
                                                 '', enterprise_api_url),
            )
